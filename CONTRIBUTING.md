...
Module integrity comes from `go.sum`, which Go verifies on every build; `sum.golang.org` is consulted when a module is first added. A first build after a dependency change needs network access to a Go proxy; afterwards it is served from your local module cache.
