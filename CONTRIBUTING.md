...
Module integrity comes from `go.sum`, which Go verifies on every build; `sum.golang.org` is consulted when a module is first added. A build needs network access to a Go proxy only for modules not already in your local module cache; anything cached is served from there.
