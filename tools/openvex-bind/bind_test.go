...
func TestBindPassesCuratedFieldsThrough(t *testing.T) {
	...
	// projection back to the reviewed file; @id and tooling are the two fields
	// that change, because the projection is a different document from its
	// source and is generated by this tool.
...
// TestBindNormalizesTooling covers NVIDIA/aicr#2706: `tooling` reached the
// signed attestation of all seven released images as an 8,010 byte revision
// changelog. The projection now reports this tool regardless of what the source
// carries, so committed prose cannot become published output no matter how the
// source drifts. The oversized fixture is the regression: a pass-through
// implementation republishes it.
func TestBindNormalizesTooling(t *testing.T) {
	t.Parallel()
	const narrative = "v15 is remediation rather than suppression and removes all seven openssl statements"
	tests := []struct {
		name    string
		tooling string
	}{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			source := strings.Replace(sourceDocument, `"tooling": "manual",`, tc.tooling, 1)
			result, err := Bind([]byte(source), Options{Image: benchImage, Digest: amd64Digest})
			if err != nil {
				t.Fatalf("Bind() error = %v", err)
			}
			if got := decode(t, result.Document)["tooling"]; got != projectionTooling {
				t.Errorf("projection tooling = %v, want %q", got, projectionTooling)
			}
			if bytes.Contains(result.Document, []byte(narrative)) {
				t.Error("source narrative reached the projection")
			}
		})
	}
}
...
func TestBindCommittedDocument(t *testing.T) {
	...
	// openvex-guard.sh rejects any document-level string over 256 bytes, and it
	// runs on the projection just before `cosign attest`. Catching an
	// over-long projectionTooling here fails a unit test instead of a release.
	if len(projectionTooling) > 256 {
		t.Errorf("projectionTooling is %d bytes; openvex-guard.sh caps document-level strings at 256",
			len(projectionTooling))
	}
