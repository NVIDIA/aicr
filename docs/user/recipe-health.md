...
> **Signer trust class.** When a recipe coordinate has committed evidence pointers, the Evidence cell renders the allowlist trust class of the latest valid attested run (`first-party`, `community`, or `partner`) alongside the deep-link. The signer classes are pinned in [`recipes/evidence/allowlist.yaml`](https://github.com/NVIDIA/aicr/blob/main/recipes/evidence/allowlist.yaml) — `first-party` is the project's own UAT workflows, `community` is individual external contributors, and `partner` is vetted partner organizations. If no allowlisted evidence pointer is committed for a coordinate, the cell renders the deep-link without a class suffix.
