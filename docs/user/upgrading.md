...
Keep the recipe you deployed from. It is the only record of where your cluster came from.

```shell
aicr upgrade-check --from old-recipe.yaml --to new-recipe.yaml --deployer helm
```

If you did not keep it, omit `--to` instead and ask the other useful question:
...
- **It does not read your cluster.** The comparison is between two artifacts. If your cluster has drifted from the recipe you think you deployed, the check compares the artifacts you gave it, not reality. Reading installed Helm release inventory is tracked in [#2531](https://github.com/NVIDIA/aicr/issues/2531).
