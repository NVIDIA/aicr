...
| `gb200` | `eks` | `p6e-gb200.36xlarge` (4 GPUs per K8s node) | Deploys; floors are sized for this shape and are not normalized for GPU count, so another GB200 shape can false-fail a healthy run. The NVLS floor targets a reported ~770 GB/s NVL72 baseline that is not yet confirmed as comparable busbw for a 2-node pair. |
