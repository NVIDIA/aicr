...
- **`helm`** — `deploy.sh` runs the readiness folder with `helm upgrade --install --wait --wait-for-jobs`. Helm's own `--timeout` is derived by the bundler from the gate's `--max-wait` plus a buffer, so the gate owns the deadline (Helm never preempts it).
