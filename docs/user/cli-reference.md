...
| `--emit-attestation` | | string | | Directory to write a recipe-evidence attestation bundle, predicateType v3, with a content-only recipe digest independent of the recipe's profile or the aicr binary version that produced it (signed when `--push` is set, unless `--no-sign`). The bundle is minimized by default, see `--full`. See [ADR-007](../design/007-recipe-evidence.md). |
