...
| **lws** | LeaderWorkerSet (LWS) controller — Kubernetes SIGs API for deploying leader/worker pod groups as a single unit of replication, the standard primitive for multi-node inference serving (one leader plus N workers scheduled, scaled, and restarted together, e.g. vLLM or Dynamo multi-host). **Not installed by default**; opt in from a custom overlay by adding an explicit `componentRef` (`name: lws`, `type: Helm`, `valuesFile: components/lws/values.yaml`). The chart self-manages its webhook certificate, so it has no cert-manager dependency. Note: the `kueue` component's managed frameworks are pinned to batch/job, JobSet, and TrainJob — deploying both does not make Kueue queue LWS workloads unless you override Kueue's framework list. CLI alias: `leaderworkerset`. | [LeaderWorkerSet](https://github.com/kubernetes-sigs/lws) |
...
- **Opt-in workload APIs** (`lws`) are not installed by default. LeaderWorkerSet is registered in the component registry with pinned chart version, values, and health check, but no stock recipe references it — opting in requires an explicit `componentRef` entry (`name: lws`, `type: Helm`, `valuesFile: components/lws/values.yaml`) in a custom or external overlay.
