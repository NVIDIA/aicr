...
`recipes/components/nvsentinel/values.yaml` sets no enablement toggles at all. It carries only deployment-shaping values: `fullnameOverride`, tolerate-all scheduling so GPU-node DaemonSets land on tainted nodes, `networkPolicy.enabled: false` (the metrics policy otherwise blocks cert-manager webhook traffic in the same namespace), `platformConnector` resources, and the generic reboot provider. Everything else is the chart default.
...
**NVSentinel is included by default, not universally required.** It is an unconditional `componentRef` in `recipes/overlays/base.yaml`, and ADR-018 classifies it as `core` rather than `ops` on the rule that `ops` must be GPU-free and verifiable on CPU-only clusters. Supported EKS and OKE configurations can disable it, and the OCP overlay does. Only the AKS and GKE-COS `gpuStack` profiles make its presence mandatory — see [NVSentinel is mandatory on the profiled families](#nvsentinel-on-provider-installed-driver-platforms) below.

**Values AICR does set are platform-correctness values, not policy.** `labeler.assumeDriverInstalled` and `metadata-collector.runtimeClassName`, covered in the next section, describe facts about the target cluster that the chart cannot infer — who installs the driver, and what the RuntimeClass is named. Leaving them at chart defaults does not preserve upstream intent; it silently breaks the component. Enablement toggles are a different matter and stay upstream's call.
