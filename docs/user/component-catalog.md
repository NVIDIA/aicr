...
On overlays that already carry the profile (AKS, GKE-COS, OKE), the injection is **semantically idempotent** — the rendered `driver.enabled` value is unchanged; the resolved recipe records the override explicitly (visible in `aicr recipe -o recipe.yaml`) so the reason for the value is auditable end-to-end. The AKS default is the [AKS driver-only profile](../integrator/aks-gpu-setup.md#default-use-the-aks-driver-only-profile) (`driver.enabled=false`, `toolkit.enabled=false`, `operator.runtimeClass=nvidia-container-runtime` — the Azure default, where the node image preinstalls driver and toolkit); a `--gpu-driver none` pool needs the [GPU Operator-managed overrides](../integrator/aks-gpu-setup.md#alternative-let-gpu-operator-manage-the-driver) at bundle time.

The inverse mismatch **warns**: when the sampled GPU node has *no* NVIDIA driver loaded but the resolved overlay declares the preinstalled-driver profile (for example, the AKS driver-only default resolved against a cluster whose GPU pools were created with `--gpu-driver none`), resolution logs a warning. On AKS the warning names both ways out — flip the bundle to GPU-Operator-managed mode (`--set gpuoperator:driver.enabled=true --set gpuoperator:toolkit.enabled=true --set gpuoperator:operator.runtimeClass=nvidia --set dradriver:nvidiaDriverRoot=/run/nvidia/driver`, see the [AKS GPU Operator-managed profile](../integrator/aks-gpu-setup.md#alternative-let-gpu-operator-manage-the-driver)), or reprovision the GPU pools with the platform's default driver install and re-snapshot. On other preinstalled-profile platforms (GKE-COS, OKE) the warning recommends only the reprovisioning path — the AKS override tuple is not coherent there (on GKE-COS, `driver.enabled=true` would desynchronize the operator driver root from the COS `hostPaths.driverInstallDir`, and the read-only COS root cannot take an operator-managed driver install). Ownership overrides are bundle-time flags, so resolution cannot fail hard without cutting off the supported override path; a blocking bundle-time coherence validation is tracked in [#1757](https://github.com/NVIDIA/aicr/issues/1757).
