...
| **nvsentinel** | GPU health monitoring and automated remediation. Detects GPU errors and can cordon or drain affected nodes. **Preinstalled-driver platforms:** on AKS, GKE-COS and OKE — whose default cluster shape has no driver pod, because the driver ships in the node image or is preloaded at node-pool creation — recipes set `labeler.assumeDriverInstalled=true`. NVSentinel's labeler otherwise infers driver presence from a driver pod that never exists there, so `nvsentinel.dgxc.nvidia.com/driver.installed` is never applied and `metadata-collector` plus both `syslog-health-monitor` DaemonSets silently report 0 desired pods. Temporary workaround for [NVIDIA/NVSentinel#1583](https://github.com/NVIDIA/NVSentinel/issues/1583); it is removed once the labeler falls back to the GPU Feature Discovery driver labels. | [NVSentinel](https://github.com/NVIDIA/nvsentinel) |
