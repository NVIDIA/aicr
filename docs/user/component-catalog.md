...
**The component values AICR sets are platform-correctness values, not remediation policy.** `labeler.assumeDriverInstalled` and `metadata-collector.runtimeClassName`, covered in the next section, describe facts about the target cluster that the chart cannot infer — who installs the driver, and what the RuntimeClass is named. The recipes assign them only where the chart default does not match the cluster — the tables below show which platforms need each value and which correctly leave it unset. Where an override is needed and missing, the affected component path breaks silently rather than erroring, so deferring to the chart default there does not preserve upstream intent. Which components run is a different matter and stays upstream's call.
