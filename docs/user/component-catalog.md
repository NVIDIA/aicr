...
**NVSentinel is a required component, not an optional add-on.** It is an unconditional `componentRef` in `recipes/overlays/base.yaml`, and ADR-018 classifies it as `core` rather than `ops` on the rule that `ops` must be GPU-free and verifiable on CPU-only clusters. On the profiled families its presence is additionally locked — see [NVSentinel is mandatory on the profiled families](#nvsentinel-on-provider-installed-driver-platforms) below.
