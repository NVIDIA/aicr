...
The catalog-registration rule described under [Audit Logging and Tracing](#audit-logging-and-tracing) applies here too: a leaf overlay naming this mixin that is never registered via `--data` (or committed to `recipes/overlays/`) silently composes *without* it, and the bundle still succeeds. Confirm the generated recipe's `nvsentinel` componentRef carries `global.preflight` before bundling. The "has no effect if `nvsentinel` is disabled by the chain" caveat applies equally, as does the allowlist mechanism — `nvsentinel`'s `mixinSafeOverridePaths` entry names each `preflight.*` leaf path this mixin may set, and anything outside it fails at compose time.
