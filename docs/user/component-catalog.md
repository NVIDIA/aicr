...
| **nvcre** | NVIDIA Cluster Readiness Engine. Opt-in: not attached to shipped overlays. Runs catalog `Certification` workloads (EKS H100 NCCL all-reduce) and optional `WorkloadRun` goodput jobs. AICR remains the judge of recipe performance constraints. Shipped EKS H100 training still uses the TrainJob NCCL check. | [cluster-readiness-engine](https://github.com/NVIDIA/cluster-readiness-engine) |
