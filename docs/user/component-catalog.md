...
- **Topology-aware optional components** (`slinky-topograph`) are not installed by default and are opted into by individual leaf overlays via `dependencyRefs`. They provide supplementary topology data to a scheduler and are only useful when paired with a matching consumer.
