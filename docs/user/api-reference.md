...
| `AICR_BINARY_ATTESTATION_IDENTITY_REGEXP` | A, B | Certificate-identity pattern the server pins its own binary attestation to. Unset uses the release-workflow default (`on-tag.yaml`). A custom value MUST be confined to `NVIDIA/aicr` so it stays pinned to the NVIDIA org — containing the repo prefix is necessary but not sufficient, and top-level alternation or any pattern still matching an unrelated identity is rejected; it retargets which NVIDIA workflow attested the binary (e.g. an e2e workflow), not the org, and a value that is not so pinned fails startup. Mirrors the CLI's `--certificate-identity-regexp`. |
