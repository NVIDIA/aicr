...
| `trust.certificateIdentityRegexp` | string | Certificate identity pattern for binary attestation verification; must be *confined* to `NVIDIA/aicr` — containing the repo prefix is necessary but not sufficient, and top-level alternation or any pattern still matching an unrelated identity is rejected |
