...
1. **Both NCCL and training/goodput use `Certification`.** For EKS H100, AICR creates `nvcre.nvidia.com/v1alpha1` `Certification` with `communication/nccl-all-reduce` or `training/nemotron5-8b`, waits for a terminal condition, always deletes the CR, and remains the judge. `WorkloadRun` is CRE's ad-hoc API and is not used by these checks. Each Certification is capped at two named nodes (`nodesPerJob` and `target.nodeNames`) with `timeoutPerJob` equal to AICR's wait budget so a timed-out check does not leave work running.
   ...
   Both bounds are enforced by AICR at the call site, not by CRE: CRE's API places no upper bound on `nodesPerJob` and has no whole-Certification deadline, so AICR names the nodes rather than letting CRE select them, and deletes the CR itself. Deletion uses foreground propagation, so the Certification is retained until CRE's Workflows, TrainJobs, and pods are gone — under background propagation the CR would disappear while the jobs still held the GPUs, and the teardown wait would prove nothing. A teardown that fails or times out fails the check rather than logging a warning, because a surviving Certification keeps occupying GPU nodes and must not be reported as a pass.
