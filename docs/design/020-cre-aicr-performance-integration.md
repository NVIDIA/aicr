...
1. **Both NCCL and training/goodput use `Certification`.** For EKS H100, AICR creates `nvcre.nvidia.com/v1alpha1` `Certification` with `communication/nccl-all-reduce` or `training/nemotron5-8b`, waits for a terminal condition, always deletes the CR, and remains the judge. `WorkloadRun` is CRE's ad-hoc API and is not used by these checks. Each Certification is capped at two named nodes (`nodesPerJob` and `target.nodeNames`) with `timeoutPerJob` equal to AICR's wait budget so a timed-out check does not leave work running.
   ...
   Both bounds are enforced by AICR at the call site, not by CRE: CRE's API places no upper bound on the total node count and has no whole-Certification deadline, so AICR names the nodes rather than letting CRE select them, and tears the CR down itself. `nodesPerJob` only sizes each job group — CRE partitions every targeted node into groups of that size — so the bound that actually caps the footprint is `target.nodeNames`.

   Deleting the `Certification` does not by itself establish that the GPU work stopped. CRE's Certification controller deletes its child `Workflow`s and drops its own finalizer in the same reconcile without waiting for them, and its Job drain barrier gives up after five minutes and proceeds while pods are still running. Foreground propagation is therefore requested but treated as necessary, not sufficient: AICR walks `Certification` → `Workflow`s → `Job`s by label *before* deleting (CRE labels only Workflows with the Certification name), then waits for every `TrainJob` and non-terminal workload pod carrying those Job names to disappear. A teardown that fails or times out fails the check rather than logging a warning, because surviving work keeps occupying GPU nodes and must not be reported as a pass.
