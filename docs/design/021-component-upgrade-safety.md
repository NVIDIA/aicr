...
|---|---|---|---|
| `helm`, `helmfile` | Helm release | `<name>`, plus `<name>-pre`, `<name>-post`, `<name>-readiness` for the bundle writer's injected folders | `localformat/templates/install-local-helm.sh.tmpl:37` hands the bundle folder's name verbatim to `helm upgrade --install` |
...
| `argocd`, `argocd-helm` | Argo CD `Application`; no Helm release | `<namePrefix><name>` | `argocd/argocd.go:719` prepends `NamePrefix`, which is user-settable and need not end in a separator (`--set deployer:namePrefix=tenant-a-`, per `docs/user/cli-reference.md`) |
...
**Argo CD creates no Helm release, so it needs a reader of its own.** This ADR did not mention Argo CD's read path at all, and a Helm-only reader returns nothing for a cluster deployed with `argocd` or `argocd-helm`. A generated `Application` with a chart source (`pkg/bundler/deployer/argocd/templates/application.yaml.tmpl`) is rendered by Argo's repo-server with `helm template` and its manifests applied directly; no release record is ever written. The `Application` CRs are therefore the only evidence those components are installed, and online mode lists them too. `argocd-helm` delegates to the `argocd` deployer and emits the same Applications, so one reader covers both; its app-of-apps genuinely *is* a Helm release, which is why Helm answers first and Argo fills only what Helm left unanswered. The per-component Applications are the per-component truth.

What Argo can be asked is narrower than what Helm can. It keeps no counterpart to a chart's annotations, no revision in Helm's sense, and its status describes sync and health rather than which version is installed, so those are left empty rather than approximated. A **path-based** `Application`, which is the shape of every generated wrapper and every Kustomize component, carries no payload version anywhere in the cluster: its `targetRevision` is the bundle repository's git revision, and reporting that would claim each such component sits at `main`. Reporting no version is the correct answer, and [Decision 2](#decision-2-transition-records)'s `unversioned` is what the comparison then renders, reached by having nothing to compare rather than by inventing something to compare.
...
Both upgrade through Helm, which installs a chart's `crds/` directory on first install and never touches it again. [#2525](https://github.com/NVIDIA/aicr/issues/2525) closes the gap. The guard that reads the flag moved out of the Flux deployer into `deployer.ResolveCRDOwners`, so flux, helm, and helmfile now resolve it identically: the flag applies only to a component whose ref still points at the registry-pinned chart, because the audit it records covers that chart and no other. On a match, `localformat` emits an `apply-crds.sh` into the component's folder, `install.sh` runs it before `helm upgrade`, and the helmfile release carries a `presync` hook running the same script. The script is upgrade-only, since Helm installs `crds/` itself on first install.
