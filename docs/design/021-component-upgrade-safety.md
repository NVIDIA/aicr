...
**Correction: only `helm` bundles embed `recipe.yaml` today.** This decision was written assuming every bundle does. `pkg/bundler/bundler.go` gates the write on `Deployer() == config.DeployerHelm`, and that is its only write site, so `argocd`, `argocd-helm`, `flux`, and `helmfile` bundles carry no recipe at all. Verified by generating one bundle per deployer. The shared code path above is therefore real but currently reachable only from a recipe file or a `helm` bundle; `--from <bundle>` against the other four fails with an explicit error rather than misreading them, and this decision's own worked example (`--from ./bundles-v0.16.0` built with `--deployer argocd`) cannot run until the bundler writes the file for every deployer. Tracked in [#2753](https://github.com/NVIDIA/aicr/issues/2753); [#2531](https://github.com/NVIDIA/aicr/issues/2531) wants the same fix, since it reads the same embedded recipe to key its cluster comparison.
...
**Corrected in [#2528](https://github.com/NVIDIA/aicr/pull/2760): nothing records which deployer built a bundle, so the inference this paragraph originally specified has no input.** It read that a bundle records its deployer and that the check infers it from `--to`. No bundle artifact carries that fact: `provenance.yaml` is written only under `--vendor-charts` and describes vendored charts, not the deployer. Until a bundle carries it ([#2753](https://github.com/NVIDIA/aicr/issues/2753) is the natural place, since it already has to write `recipe.yaml` for every deployer), every caller supplies the value. It does not guess, and it does not render every path: showing an Argo CD operator the imperative "delete legacy CRs" step is the failure deployer-scoping exists to prevent, so a silent default would reintroduce it. Recipe-to-recipe in CI therefore passes `--deployer` explicitly, which the pipeline already knows because it passes the same value to `aicr bundle`.
