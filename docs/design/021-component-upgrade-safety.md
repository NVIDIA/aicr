...
**Corrected.** This decision originally named four deployers (helm, helmfile, argocd, and argocd-helm) on the strength of a single grep showing `ownsCRDs` had one consumer. That grep answers "which deployer reads the flag", which is not the same question as "which deployer strands CRDs". The two diverge on Argo CD: its repo-server renders Helm sources with `--include-crds` unless the Application sets `helm.skipCrds`, so CRDs are ordinary manifests re-applied on every sync, and it upgrades them without ever needing an opt-in. The origin issue [#2264](https://github.com/NVIDIA/aicr/issues/2264) had this right, and [the deployer table in the component catalog](../user/component-catalog.md#upgrade-uninstall-and-troubleshooting) has carried the correct version since, verified against a live GKE cluster across a 1.2.0 to 1.3.0 upgrade.

Argo CD also could not be gated even if someone wanted the symmetry: `skipCrds` suppresses CRDs on *first install* too, so gating it on `ownsCRDs` would break a fresh install of every CRD-shipping chart.

The remaining gap is helm and helmfile, both of which upgrade through Helm, which installs a chart's `crds/` directory on first install and never touches it again. [#2525](https://github.com/NVIDIA/aicr/issues/2525) closes it: `localformat` emits an `apply-crds.sh` into the folder of every component the registry marks `ownsCRDs` whose ref still points at the registry-pinned chart, `install.sh` runs it before `helm upgrade`, and the helmfile release gets a `presync` hook running the same script. Argo CD's two load-bearing properties, no `skipCrds` and `ServerSideApply=true`, are pinned by a regression test rather than reimplemented.
