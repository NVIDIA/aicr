...
| [`aicr-triage`](https://github.com/NVIDIA/aicr/blob/main/.agents/skills/aicr-triage/SKILL.md) | Triage the NVIDIA AICR project board (project 248): classify new issues, raise or lower priority (P2→P1, P1→P2), move issues between Backlog and Ready in either direction, and propose closures — comments the outcome on every issue it triages, including ones it leaves unchanged, with per-bucket confirmation and a separate opt-in for that comment batch. |
