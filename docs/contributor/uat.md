...
**Per-step results.** Besides the signed evidence pointer, each cloud pipeline uploads `uat-<cloud>-snapshot-result-<run_id>-<attempt>` on every outcome (the nvkind lane names it `uat-kind-<intent>-snapshot-result-…`). It holds `snapshot-result.json`, a [CTRF](https://ctrf.io) report written by `phase_prep` through the shared `tools/ctrf` emitter, recording the live-cluster `aicr snapshot` step as `passed` or `failed` with its duration. The file is written before the snapshot-failure debug dump, so it is present in the failure-debug bundle as well. The other phases already produce structured results of their own: `aicr validate --output` writes CTRF (`dry-run.json`, `report.json`) and `aicr evidence verify -t json` writes `evidence-result.json`.
