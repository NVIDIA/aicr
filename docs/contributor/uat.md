...
- **Evening — teardown.** `uat-daytime.yaml` fires `lifecycle=daytime-down` on an evening cron — the **only** scheduled daytime edge — to tear the daytime cluster down and release the reservation **before** the next night batch. It is an unconditional safety net: it runs whether or not anyone stood a cluster up, so a manually-provisioned daytime cluster left running never leaks into the nightly window.
...
|------|---------|--------|----------------------|
| Handoff (stand up) | **manual dispatch** (`action=up`) | `up` | `daytime-up` (provision + deploy + hold) |
| Evening teardown | cron `0 2 * * *` (UTC) **or** manual (`action=down`) | `down` | `daytime-down` (tear down + release) |

The evening teardown runs ~2h before the nightly batch opens (`0 4 * * *`), leaving margin for a ~10–15 min destroy, and fires **whether or not** a cluster was stood up — so a manually-held cluster never leaks into the nightly window.
