...
| `kind-h100` | kind (nvkind) | `[training, inference]` | `phase_train` (single-GPU); both intents gated to `>= v0.18.0` via `nightly-intent-min-versions` — the lane + os-agnostic coordinate fix (#1851) postdate v0.17.0, so only `main` runs nvkind nightly until v0.18.0 ships |
