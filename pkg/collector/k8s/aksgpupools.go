...
// aggregate to Mixed and reject a valid driver-only cluster.
//
...
//     (gpuProfile.nvidia present) projects as Managed. Neither matches a
//     declared constraint, so resolution fails closed with the observed
//     value as the actual.
...
// profile decision.
func ProjectAKSGPUPools(path string) (measurement.Subtype, error) {
	pools, err := readAKSAgentPools(path)
	if err != nil {
	...
	for _, pool := range pools {
		...
		case len(pool.GPUProfile.NVIDIA) > 0 && string(pool.GPUProfile.NVIDIA) != "null":
...
func readAKSAgentPools(path string) ([]aksAgentPool, error) {
	raw, err := readBoundedPoolsFile(path, "AKS GPU pools", defaults.MaxAKSGPUPoolsBytes)
	if err != nil {
		return nil, err
	}
