...
func TestProjectAKSGPUPools(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			subtype, err := ProjectAKSGPUPools(writeAKSPoolsFile(t, tt.content))
			if err != nil {
				t.Fatalf("ProjectAKSGPUPools() error = %v", err)
			}
...
func TestProjectAKSGPUPoolsFailsLoud(t *testing.T) {
	tests := []struct {
		...
		{
			// The Lstat gate runs first, so a missing file surfaces there.
			name:    "missing file",
			path:    func(t *testing.T) string { t.Helper(); return filepath.Join(t.TempDir(), "absent.json") },
			wantErr: "failed to inspect",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := ProjectAKSGPUPools(tt.path(t))
			if err == nil || !strings.Contains(err.Error(), tt.wantErr) {
				t.Fatalf("ProjectAKSGPUPools() error = %v, want containing %q", err, tt.wantErr)
			}
		})
	}
}

// TestReadRejectsNonRegularFile pins the Lstat gate: io.LimitReader bounds
// bytes, not time, so a FIFO or symlink must be rejected before open.
func TestReadRejectsNonRegularFile(t *testing.T) {
	dir := t.TempDir()
	_, dirErr := ProjectAKSGPUPools(dir)
	...
	_, linkErr := ProjectAKSGPUPools(link)
...
func TestReadingShapeMatchesProfileContract(t *testing.T) {
	subtype, err := ProjectAKSGPUPools(writeAKSPoolsFile(t,
		`[{"name":"gpu1","vmSize":"Standard_ND96isr_H100_v5","gpuProfile":{"driver":"Install"}}]`))
	if err != nil {
		t.Fatalf("ProjectAKSGPUPools() error = %v", err)
	}
