...
Trust levels:
  ...
  unverified  Checksums valid, no attestation files (--skip-attestation was used)
...
func runBundleVerifyCmd(ctx context.Context, cmd *cli.Command) error {
	...
	minLevelStr := cmd.String("min-trust-level")
	if minLevelStr == "max" {
		// "max" auto-detects: the result's trust level IS the maximum achievable.
		// Verify it's at least unverified (checksums must pass).
		if !result.TrustLevel.MeetsMinimum(verifier.TrustUnverified) {
			return errors.New(errors.ErrCodeInvalidRequest,
				fmt.Sprintf("bundle verification failed: trust level is %q", result.TrustLevel))
		}
	} else if minLevelStr != "" {
		minLevel, parseErr := verifier.ParseTrustLevel(minLevelStr)
		if parseErr != nil {
			return parseErr
		}
		if !result.TrustLevel.MeetsMinimum(minLevel) {
			return errors.New(errors.ErrCodeInvalidRequest,
				fmt.Sprintf("trust level %q does not meet minimum %q", result.TrustLevel, minLevel))
		}
	}

	// Check required creator identity
	requiredCreator := cmd.String("require-creator")
	if requiredCreator != "" && result.BundleCreator != requiredCreator {
		return errors.New(errors.ErrCodeInvalidRequest,
			fmt.Sprintf("bundle creator %q does not match required %q", result.BundleCreator, requiredCreator))
	}

	// Check required tool version
	requiredVersion := cmd.String("require-tool-version")
	if requiredVersion != "" && result.ToolVersion != requiredVersion {
		return errors.New(errors.ErrCodeInvalidRequest,
			fmt.Sprintf("tool version %q does not match required %q", result.ToolVersion, requiredVersion))
	}

	return nil
}

func outputText(r *verifier.VerifyResult) {
	...
	if r.IdentityPinned {
		fmt.Println("  ✓ Identity pinned to NVIDIA CI")
	}
	...
	if r.TrustLevel.MeetsMinimum(verifier.TrustUnverified) {
