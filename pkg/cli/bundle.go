...
func parseBundleCmdOptions(cmd *cli.Command) (*bundleCmdOptions, error) {
	opts := &bundleCmdOptions{
		...
		skipAttestation: cmd.Bool("skip-attestation"),
...
`,
	...
	(format: bundler:path.to.field=value, e.g., --set gpuoperator:gds.enabled=true)`,
			...
			&cli.BoolFlag{
				Name:  "skip-attestation",
				Usage: "Skip bundle attestation and binary verification",
			},
			kubeconfigFlag,
			dataFlag,
...
func runBundleCmd(ctx context.Context, cmd *cli.Command) error {
	...
	cfg := config.NewConfig(
		...
		config.WithSkipAttestation(opts.skipAttestation),
	...
	attester, err := selectAttester(ctx, opts.skipAttestation)
	if err != nil {
		return err
	}
...
func selectAttester(ctx context.Context, skipAttestation bool) (attestation.Attester, error) {
	if skipAttestation {
	...
	if requestURL != "" && requestToken != "" {
		...
		if err != nil {
			return nil, errors.Wrap(errors.ErrCodeUnavailable,
				"failed to fetch OIDC token for attestation; use --skip-attestation to skip", err)
		}
	...
	oidcToken, err := attestation.FetchInteractiveOIDCToken()
	if err != nil {
		return nil, errors.Wrap(errors.ErrCodeUnavailable,
			"bundle attestation requires authentication; use --skip-attestation to skip", err)
	}
	return attestation.NewKeylessAttester(oidcToken), nil
}
