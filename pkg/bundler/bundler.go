...
type DefaultBundler struct {
	...
	// Attester signs bundle content. Use NoOpAttester for --skip-attestation.
...
func (b *DefaultBundler) makeHelmBundle(ctx context.Context, recipeResult *recipe.RecipeResult, componentValues map[string]map[string]any, dir string, start time.Time) (*result.Output, error) {
	...
	attestFiles, err := b.attestBundle(ctx, dir, dataFiles)
	if err != nil {
		return nil, err
	}
...
func (b *DefaultBundler) attestBundle(ctx context.Context, dir string, dataFiles []string) ([]string, error) {
	if b.Attester == nil || b.Config.SkipAttestation() {
		return nil, nil
	}
	...
	if err != nil {
		// If checksums don't exist (IncludeChecksums=false), skip attestation
		slog.Debug("skipping attestation: checksums not available", "error", err)
		return nil, nil
	}

	// Build attestation subject
	...
	bundleAttestPath := fmt.Sprintf("%s/%s", dir, attestation.BundleAttestationFile)
	...
	// Copy binary attestation into bundle (if available)
	if binaryPath, execErr := os.Executable(); execErr == nil {
		...
		if findErr == nil {
			...
			if readErr == nil {
				destPath := fmt.Sprintf("%s/%s", dir, attestation.BinaryAttestationFile)
