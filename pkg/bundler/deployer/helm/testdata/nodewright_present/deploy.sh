...
nodewright_available=$(kubectl get deploy -n skyhook -l app.kubernetes.io/name=nodewright,control-plane=controller-manager -o jsonpath='{.items[0].status.availableReplicas}' 2>/dev/null || echo "0")
if [[ "${nodewright_available}" == "0" || -z "${nodewright_available}" ]]; then
  ...
  # If runtimeRequiredTaint is not set, cover both keys the operator recognises:
  # its chart default (nodewright.nvidia.com, v0.18.0+) and the legacy default
  # (skyhook.nvidia.com) node pools may still pre-taint with.
  NODEWRIGHT_TAINT_KEYS="nodewright.nvidia.com skyhook.nvidia.com"
  ...
  if [[ -n "${nodewright_dir}" && -f "${nodewright_dir}/values.yaml" ]]; then
    ...
    if [[ -n "${custom_taint_line}" ]]; then
      ...
      if [[ -n "${taint_value}" ]]; then
        # Handle both key=value:effect and key:effect formats. The legacy key
        # stays in the set: the operator still removes it, and the validator's
        # taint gate waits on it alongside the configured key.
        custom_key="${taint_value%%=*}"
        custom_key="${custom_key%%:*}"
        NODEWRIGHT_TAINT_KEYS="${custom_key}"
        if [[ "${custom_key}" != "skyhook.nvidia.com" ]]; then
          NODEWRIGHT_TAINT_KEYS="${custom_key} skyhook.nvidia.com"
        fi
      fi
    fi
  fi
  for NODEWRIGHT_TAINT_KEY in ${NODEWRIGHT_TAINT_KEYS}; do
