...
function remove_stale_nodewright_taints() {
  ...
  local available
  if ! available=$(kubectl get deploy -n "${ns}" -l app.kubernetes.io/name=nodewright,control-plane=controller-manager -o jsonpath='{.items[0].status.availableReplicas}' 2>&1); then
    _warn_line "could not query the nodewright-operator Deployment in ${ns}; skipping stale-taint cleanup: ${available}"
    return 0
  fi
