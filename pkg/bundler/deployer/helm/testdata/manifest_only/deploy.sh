...
#   --no-wait      Skip Helm chart-level wait where AICR uses --wait (keeps
#                  --timeout for hooks). Correctness gates such as
#                  gpu-operator-post (issue #980) override --no-wait and
#                  always wait — they exist to prevent a known race, not as
#                  a readiness convenience.
...
for dir in "${SCRIPT_DIR}"/[0-9][0-9][0-9]-*/; do
  ...
  COMPONENT_EXTRA_WAIT_ARGS=""
  # COMPONENT_FORCE_WAIT marks correctness gates that must wait even
  # when the operator passed --no-wait. --no-wait is a readiness-
  # convenience flag, not a correctness-bypass flag; components that
  # exist specifically to close a known race override it.
  COMPONENT_FORCE_WAIT="false"
  case "${name}" in
    kai-scheduler)
      ...
      ;;
    gpu-operator)
      # Vendored-mode coverage for the DRA kubelet-plugin rollout
      # hook (issue #980). With `--vendor-charts`, localformat
      # collapses recipe-side raw manifests into the primary
      # gpu-operator wrapper chart (no `gpu-operator-post` folder is
      # emitted), so the DRA hook Job ships under THIS release with
      # its author-declared `helm.sh/hook: post-install,post-upgrade`
      # annotations preserved. Helm waits for hook Jobs automatically
      # under `--wait` (so --wait-for-jobs is not needed here), but
      # the outer `--timeout 10m` is shorter than the Job's worst
      # case (15m kubectl wait + 5m rollout status = 20m). 25m
      # adds 5m headroom.
      #
      # In non-vendored gpu-operator deployments this raises the cap
      # on a release that typically completes in 3–5 min; the cap is
      # a hard ceiling, not a delay, so the over-provisioning has no
      # success-path cost.
      COMPONENT_HELM_TIMEOUT="25m"
      ;;
    gpu-operator-post)
      # Non-vendored-mode coverage for the DRA kubelet-plugin rollout
      # hook (issue #980). The Job can spend up to 15m in
      # `kubectl wait` for per-node driver migration and another 5m
      # in `kubectl rollout status`. The outer Helm timeout must
      # exceed that worst case; 25m adds 5m headroom.
      #
      # --wait-for-jobs is load-bearing here: `stripHelmHooks` removes
      # the manifest's helm.sh/hook annotations on this non-vendored
      # path so the rollout Job ships as a regular chart resource.
      # Without --wait-for-jobs, `helm install --wait` returns as soon
      # as the Job is created and downstream releases such as
      # nvidia-dra-driver-gpu can install while the migration wait /
      # restart is still running — defeating the cross-deployer
      # fail-closed gating this Job exists to provide.
      #
      # COMPONENT_FORCE_WAIT="true" prevents --no-wait from bypassing
      # this gate. Because hooks were stripped on this path, the Job
      # is a regular chart resource; without --wait --wait-for-jobs,
      # helm returns the moment the Job is created and the race
      # window reopens.
      COMPONENT_HELM_TIMEOUT="25m"
      COMPONENT_EXTRA_WAIT_ARGS="--wait-for-jobs"
      COMPONENT_FORCE_WAIT="true"
      ;;
  esac
  if [[ "${NO_WAIT}" == "true" && "${COMPONENT_FORCE_WAIT}" != "true" ]]; then
    COMPONENT_WAIT_ARGS="--timeout ${COMPONENT_HELM_TIMEOUT}"
  else
    COMPONENT_WAIT_ARGS="--wait --timeout ${COMPONENT_HELM_TIMEOUT} ${COMPONENT_EXTRA_WAIT_ARGS}"
    if [[ "${NO_WAIT}" == "true" ]]; then
      echo "  (correctness gate — overriding --no-wait for ${name})"
    fi
  fi
