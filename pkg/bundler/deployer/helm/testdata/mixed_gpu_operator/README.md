...
> **Correctness-gate exception:** `--no-wait` skips only readiness-convenience waits. Components that exist specifically to close a known race — currently the `gpu-operator-post` release that re-rolls the NVIDIA DRA kubelet plugin after gpu-operator's driver migration ([issue #980](https://github.com/NVIDIA/aicr/issues/980)) — always run with `--wait --wait-for-jobs` regardless of `--no-wait`. The script prints `(correctness gate — overriding --no-wait for <name>)` when this happens.
