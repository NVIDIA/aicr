...
test_validate() {
  ...
  # Check the output file has the correct phase in the results
  if [ -f "$deployment_result" ] && jq -e '.results.tests[] | select(.suite[] == "deployment")' "$deployment_result" > /dev/null 2>&1; then
  ...
  # Check the output file has the correct phase in the results
  if [ -f "$performance_result" ] && jq -e '.results.tests[] | select(.suite[] == "performance")' "$performance_result" > /dev/null 2>&1; then
